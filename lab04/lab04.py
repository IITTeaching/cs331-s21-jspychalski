...
class ArrayList:
    ...
    def __str__(self):
        ...
        if self.len == 0: return "[]"
            ...
            for x in range(self.len):
                stringFinal += (str(self.data.__getitem__(x)) + ", ")
            return stringFinal[:-2] + "]"
        ### END SOLUTION
    ...
    def append(self, value):
        ...
        ### BEGIN SOLUTION
        larger = ConstrainedList(self.len+1)
        for i in range(self.len):
            larger[i] = self.data[i]
        larger[self.len] = value
        self.len += 1
        self.data = larger
        ### END SOLUTION
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        if idx > self.len: raise IndexError
        else:
            self.append(0)
            for x in range(self.len-1,idx,-1):
    ...
    def pop(self, idx=-1):
        ...
        if idx > self.len or idx < 0: 
            raise IndexError(self.len-idx)
        else:
            val = self.data[idx]
            for x in range(idx,self.len-1):
                self.data[x] = self.data[x+1]
    ...
    def __eq__(self, other):
        ...
        ### BEGIN SOLUTION
        if not type(other) == ArrayList or not len(other) == self.len:
            return False
        else:
            for i in range(self.len):
                if not other[i] == self.data[i]:
                    return False
            return True
        ### END SOLUTION
    ...
    def __add__(self, other):
        ...
        of other."""
        ### BEGIN SOLUTION
        a = ArrayList(self.len + len(other))
        for i in range(self.len):
            a[i] = self.data[i]
        for x in range(len(other)):
            a[self.len + x] = other[x]
        return a
        ### END SOLUTION
